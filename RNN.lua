...
cmd:text('Options:')
-- The main important options in my experience
...
--[[ output layer ]]--
cmd:option('--softmaxtree', true, 'use SoftmaxTree instead of the inefficient (full) softmax')
...
cmd:option('--trainEpochSize', 40000, 'number of train examples seen between each epoch')
cmd:option('--validEpochSize', 2400, 'number of valid examples used for early stopping and cross-validation') 
...
cmd:option('--tiny', true, 'use a tiny (1/100th) subset of the training set (BillionWors only)')
-- TextSource
...
-- Converting our some of our desired parameters for recurrance into a form useable by the program
cmd:text()
...
--[[Data]]--
--Based on the options selected will use a different dataset or subset of a dataset
...
--[[Saved experiment]]--
-- CUDA stuff, was not able to test personally
if opt.xpPath ~= '' then
   if opt.cuda then
      require 'cunnx'
...
-- language model
lm = nn.Sequential() -- Naming the model

local inputSize = opt.hiddenSize[1]
for i,hiddenSize in ipairs(opt.hiddenSize) do 
    --Linear layer if wanted
...
if not opt.trainOnly then
   valid = dp.Evaluator{ -- Create the validation aspect of the model, if selected
      feedback = dp.Perplexity(),  
      sampler = torch.isTypeOf(ds, 'dp.TextSource') 
   ...
   tester = dp.Evaluator{ -- Create the evaluation aspect of the model, if selected
      feedback = dp.Perplexity(),  
      sampler = torch.isTypeOf(ds, 'dp.TextSource') 
