...
cmd:text('Options:')
-- The options I found to be important. Basically the ones that deal with the convolution itself and a few generally useful things
cmd:option('--channelSize', '{32,64}', 'Number of output channels for each convolution layer.')
cmd:option('--kernelSize', '{6,6,6,6}', 'kernel size of each convolution layer. Height = Width')
cmd:option('--kernelStride', '{1,1,1,1}', 'kernel stride of each convolution layer. Height = Width')
cmd:option('--poolSize', '{3,3,3,3}', 'size of the max pooling of each convolution layer. Height = Width')
...
-- These options don't seem relevant for the most part, but they are there for trying things out or if a GPU is being used
cmd:option('--learningRate', 0.1, 'learning rate at t=0')
cmd:option('--lrDecay', 'linear', 'type of learning rate decay : adaptive | linear | schedule | none')
cmd:option('--minLR', 0.00001, 'minimum learning rate')
cmd:option('--saturateEpoch', 30, 'epoch at which linear decayed LR will reach minLR')
...
-- Converting our desired parameters for convolution into a form useable by the program
...
--[[preprocessing]]--
-- Trying to do this greatly increased the run time for me without improving performance; may be better if GPU being used
...
--[[Model]]--
cnn = nn.Sequential() -- Naming the model
...
for i=1,#opt.channelSize do
   ...
   cnn:add(nn.SpatialConvolution( -- actually creating the convolutional layer(s)
   ...
   cnn:add(nn[opt.activation]()) -- adding activation function to the model
   if opt.poolSize[i] and opt.poolSize[i] > 0 then
      cnn:add(nn.SpatialMaxPooling( -- assuming it is being used, this is where pooling is implemented
...
-- dense hidden layers (if using that option)
...
cnn:add(nn.Linear(inputSize, #(ds:classes()))) -- Input is the output size of previous layer, output is number of classes we are sorting into. For MNIST, that would be 10.
...
valid = ds:validSet() and dp.Evaluator{ -- Create the validation aspect of the model
   feedback = dp.Confusion(),  
   sampler = dp.Sampler{batch_size = opt.batchSize}
}
test = ds:testSet() and dp.Evaluator{ -- Create the testing aspect of the model
   feedback = dp.Confusion(),
   sampler = dp.Sampler{batch_size = opt.batchSize}
}
